...
export const register = async (username: string, email: string, passwordHash: string) => {
  ...
  const accessToken = tokenService.signAccessToken({ userId: user.id as string, email: user.email });
  const refreshToken = tokenService.signRefreshToken({ userId: user.id as string });
...
export const refresh = async (refreshToken: string) => {
  ...
  const newAccessToken = tokenService.signAccessToken({ userId: user.id as string, email: user.email });
  const newRefreshToken = tokenService.signRefreshToken({ userId: user.id as string });
